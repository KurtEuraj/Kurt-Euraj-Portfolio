...
function ProjectCard({img, name, link, description, tech}) {
  ...
  return (
    <div className="flex flex-col items-center max-w-[450px] gap-[25px] p-[25px] rounded-md project-card" >
        ...
        <div className="h-auto flex flex-col items-start gap-[8px]">
            <div className='mt-7' >
                <h4 className="text-white text-[24px] mb-1" >The Problem</h4>
                <p className="h-auto text-white text-[16px]">{description.problem}</p>
            </div>
            <div className='mt-7' >
                <h4 className="text-white text-[24px] mb-1">The Solution</h4>
                <p className="h-auto text-white text-[16px]">{description.solution}</p>
            </div>
            {/* <button className='py-1 px-4 transition-all text-white rounded-full bg-[#333] cursor-pointer self-end hover:bg-[#444]' onClick={() => setShowMore(showMore => !showMore)} >{!showMore ? "More" : "Less"}</button> */}
        </div>
        <div className="self-start mt-7 mb-[20px]" >
